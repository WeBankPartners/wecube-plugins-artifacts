package com.webank.plugins.artifacts.support.s3;
...
public class S3Client {
    ...
    public String uploadBinaryToS3(String bucketName, String s3keyName, byte[] binary)
        ...
        try (FileOutputStream fos = new FileOutputStream(tempFile)) {
            ...
            if (!tempFile.delete()) {
                throw new PluginException("Delete temporary file failed");
            }
        } catch (IOException e) {
    ...
    public String uploadFile(String bucketName, String s3KeyName, File file) {
        ...
        if (checkFileExists(bucketName, s3KeyName)) {
            throw new PluginException(String.format("File[%s] already exists", s3KeyName));
        }
    ...
    public String getUrlFromS3(String bucketName, String s3KeyName) {
        ...
        String url = s3Client.generatePresignedUrl(httpRequest).toString();
        return url;
    }
}
