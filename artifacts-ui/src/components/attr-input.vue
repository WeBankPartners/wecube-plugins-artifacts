        ...
        return data
          .map(_ => {
            ...
            if (_.parentRs) {
              ...
              if (
                this.ciTypeAttributeObj[_.parentRs.attrId].inputType !== "ref" || this.ciTypeAttributeObj[_.parentRs.attrId].inputType !== "multiRef"
              ) {
  ...
  methods: {
    ...
    formatAllCiType() {
      this.allCiTypes.forEach(_ => {
        this.allCi = this.allCi.concat(_.ciTypeId ? _ : [])
      });
    },
    inputHandler(v) {
      if (!v.data) {
