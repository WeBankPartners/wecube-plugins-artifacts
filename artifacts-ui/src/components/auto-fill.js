  ...
  methods: {
        ...
        <div slot="content" class="auto-fill-options">
          {
            this.options.map(_ => _.type === "option" ? (
              <div class={_.class} onClick={_.fn}>{_.nodeName}</div>
            ) : (
              <hr/>
            ))
          }
    ...
    async getRefData(ruleIndex, attrIndex, ciTypeId) {
      ...
      if (refFroms.status === "OK" && ciAttrs.status === "OK") {
        // 下拉框添加被引用的CI的选项
        refFroms.data.length && this.options.push({
          type: "line"
        })
        this.options = this.options.concat(refFroms.data.map(_ => {
        ...
        // 下拉框添加属性及引用的CI的选项
        ciAttrs.data.length && this.options.push({
          type: "line"
        })
        this.options = this.options.concat(ciAttrs.data.map(_ => {
    ...
    showEnumOptions(ruleIndex, attrIndex) {
      this.currentRule = ruleIndex
      this.currentAttr = attrIndex
      this.options = this.options.concat(this.enumCodes.map(_ => {
        return {
          type: "option",
    ...
    renderExpression(val, i) {
      // type === rule 时，链式属性表达式
      let result = JSON.parse(val.value).map((_, attrIndex) => {
        let isLegal = true
        if (attrIndex === JSON.parse(val.value).length - 1) {
          const lastInputType = JSON.parse(val.value)[attrIndex].parentRs ? this.ciTypeAttrsObj[JSON.parse(val.value)[attrIndex].parentRs.attrId].inputType : ""
          if (lastInputType === "ref" || lastInputType === "multiRef" || !lastInputType) {
            isLegal = false
          } else if (lastInputType === "select" || lastInputType === "multiSelect") {
            isLegal = !!JSON.parse(val.value)[attrIndex].enumCodeAttr
          }
        }
        let filterNode = ""
        if (_.filters) {
          filterNode += "["
          filterNode += _.filters.map((filter, filterIndex) => {
        ...
        const ciTypeName = this.ciTypesObj[_.ciTypeId].name
        if (!_.parentRs) {
          return `${ciTypeName}${filterNode}`.split("").map(_ => {
            const classList = {
              "auto-fill-span": true,
            ...
            return <span class={className} index={i} attr-index={attrIndex}>{ _ }</span>
          })
        } else {
          const ref = _.parentRs.isReferedFromParent === 1 ? "->" : "<-"
          ...
          if (this.ciTypeAttrsObj[_.parentRs.attrId].inputType === "ref" || this.ciTypeAttrsObj[_.parentRs.attrId].inputType === "multiRef") {
            return ` ${ref}(${ciTypeName})${attrName}${filterNode}`.split("").map(_ => {
              const classList = {
                "auto-fill-span": true,
                "auto-fill-hover": this.hoverAttr === attrIndex + "" && this.hoverSpan === i + "",
                "auto-fill-current-node": this.currentRule === i + "" && this.currentAttr === attrIndex + "",
            ...
            return ` ${ref}${attrName}${enumCode}${filterNode}`.split("").map(_ => {
              const classList = {
                "auto-fill-span": true,
                "auto-fill-hover": this.hoverAttr === attrIndex + "" && this.hoverSpan === i + "",
                "auto-fill-current-node": this.currentRule === i + "" && this.currentAttr === attrIndex + "",
      ...
      return [
        <span class={`auto-fill-span auto-fill-braces${this.hoverSpan === i + "" ? " contains" : ""}`} index={i}>{ "{ " }</span>,
        result,
        <span class={`auto-fill-span auto-fill-braces${this.hoverSpan === i + "" ? " contains" : ""}`} index={i}>{ " }" }</span>
      ]
    },
      ...
      if (this.activeDelimiterIndex === i + "") {
        ...
        result = (
          <Input
            ...
            onInput={v => this.onDelimiterInput(v)}
            value={val}
          />
        )
      } else {
        result = [
          ...
          val.split("").map(_ => {
            return (
              <span
                class={`auto-fill-span${this.hoverSpan === i + "" ? " hover" : ""}`}
                index={i}
              >{ _ }</span>
            )
          }),
    ...
    confirmDelimiter(i) {
      if (this.activeDelimiterValue === "") {
        // 如果输入框没有值，则在 this.autoFillArray 中删掉该项
        this.autoFillArray.splice(i, 1)
      } else {
        this.autoFillArray[i].value = this.activeDelimiterValue
      ...
      this.activeDelimiterValue = ""
      this.handleInput()
    },
    onDelimiterInput(v) {
      this.activeDelimiterValue = v
    },
    renderAddRule() {
      return [
        <Icon class="auto-fill-add" type="md-add-circle" />,
        !this.autoFillArray.length && (<span class="auto-fill-add auto-fill-placeholder">{this.$t("artifacts_auto_fill_filter_placeholder")}</span>)
      ]
    },
    initAutoFillArray() {
    ...
    renderModal() {
      ...
      return (
        <Modal
          ...
          title={this.$t("artifacts_auto_fill_filter_modal_title")}
        ...
        >
          {
            this.filters.map((_, i) => (
              <div class="auto-fill-filter-li">
                ...
                <Select
                  value={_.name}
                  onInput={v => this.changeFilter(v, i)}
                  class="auto-fill-filter-li-select title"
                >
                  {
                    this.filterCiAttrs.map(attr => (
                      <Option key={attr.ciTypeAttrId} value={attr.propertyName}>{attr.name}</Option>
                    ))
                  }
                </Select>
                <Select
          ...
          <Button
            type="primary"
            long
            onClick={() => this.filters.push(emptyFilter)}
          >
    ...
    confirmFilter() {
      const filters = this.filters.filter(_ => _.name && _.operator).map(_ => {
        if (_.operator === "in") {
